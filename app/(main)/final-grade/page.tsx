...
import { FinalGradeData, FinalGradeCourse, JWTPayload, Faculty, Prodi, KampusItem } from "@/lib/types";
...
import { ChevronRight as ChevronRightIcon, Plus, X, Filter, GraduationCap, Building2, BookOpen, Book, RefreshCw } from "lucide-react";
        ...
        if (token && !JWTAuth.isTokenExpired(token)) {
            ...
            if (payload) {
                if (payload.admin !== undefined && payload.username !== undefined) {
                    ...
                    console.error("JWT Payload does not contain 'admin' or 'username' properties.");
                }
            } else {
...
const calculateBoxplotStats = (data: number[]) => {
    ...
    const q1 = (arr: number[]) => median(arr.slice(0, Math.floor(arr.length / 2)));
    ...
    const nonOutliers = sortedData.filter((value) => value >= lowerBound && value <= upperBound);
...
const BoxplotBar = (props: any) => {
    const { x, y, width, q1, q3, median, min, max } = props;
...
export default function FinalGradesPage() {
    const [rawData, setRawData] = useState<FinalGradeData[]>([]);
    ...
    useEffect(() => {
        ...
        if (savedFilters) {
            try {
                ...
                const newFilters = {
                    kampus: parsed.kampus && typeof parsed.kampus === 'string' ? parsed.kampus : "TEL-U BANDUNG",
                    kampusId: parsed.kampusId && typeof parsed.kampusId === 'string' ? parsed.kampusId : "bdg",
                    fakultas: parsed.fakultas && typeof parsed.fakultas === 'string' ? parsed.fakultas : "",
            ...
            if (appliedFilters.courseId) {
                ...
                try {
                    ...
                    if (response.status && Array.isArray(response.data)) {
                        ...
                        console.error('Final grades data is not an array:', response.data);
                        setRawData([]);
                    }
                } catch (err: any) {
                ...
                console.log('No courseId provided, setting rawData to empty');
                setRawData([]);
            }
        };

        if (!isUserLoading && isAuthorized && hasApplied) {
            fetchFinalGrades();
        }
    }, [appliedFilters.courseId, isUserLoading, isAuthorized, hasApplied]);

    const chartData = useMemo(() => rawData ? formatDataForChart(rawData) : [], [rawData]);
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        <div className="flex items-center gap-1">
                            <div className="flex items-center gap-2">
                                ...
                                <Select value={selectedKampus} onValueChange={handleKampusChange}>
                        ...
                        {currentLevel >= 4 && selectedProdiId && (
                            <div className="flex items-center gap-1">
                                <ChevronRightIcon className="h-4 w-4 text-gray-400" />
                                <div className="flex items-center gap-2">
                                ...
                                <ChartContainer config={{}} className="min-h-[300px] w-full">
                                    <ResponsiveContainer width="100%" height={400}>
                                        <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                            ...
                                            <XAxis dataKey="course_name" tickFormatter={(name) => name.split(" ")[0]} />
                                            <YAxis domain={[0, 100]} />
                                            <ChartTooltip content={<ChartTooltipContent />} />
                                            <Bar dataKey="q3" shape={BoxplotBar} />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </ChartContainer>
                            ) : (
                                <div className="flex justify-center items-center h-64">
                                    <p className="text-gray-500">Tidak ada data yang tersedia untuk pilihan ini.</p>
                                </div>
                            )}
                        </CardContent>
                    </Card>
